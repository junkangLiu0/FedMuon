...
#from optimizer.FastSAM import FastSAM
#from optimizer.mars import MARS
#from muon import MuAdam
...
#from muon import MuonWithAuxAdam, Muon
...
if CNN == 'cct':
    ...
    def ConvNet200(num_classes=200):
        return cct_7_3x1_32_c100(pretrained=False, progress=True, num_classes=2000)
...
def quantize_to_float8(momentum):
    """
    Simulate the quantization of a tensor to 'float8' by converting it to int8.
    This is a simplified version as 'float8' isn't directly supported in PyTorch.
    """
    # 计算缩放因子，将数据映射到 [-128, 127] 范围
    scale = momentum.abs().max() / 127.0  # 对应于 int8 范围 [-128, 127]

    # 量化：将数据映射到 int8 范围
    quantized_momentum = torch.round(momentum / scale).to(torch.int8)

    return quantized_momentum, scale


def dequantize_from_float8(quantized_momentum, scale):
    """
    Convert the quantized 'float8' (simulated via int8) back to float32.
    """
    return quantized_momentum.to(torch.float32) * scale  # 反量化回 float32
...
class DataWorker(object):
    ...
    def update_FedMuonC(self, weights, E, index,ps_c, lr):
        ...
        param_groups = [
            dict(params=hidden_weights, use_muon=True,
                 lr=lr*(1-args.gamma)*args.alpha, weight_decay=0.01,momentum=0.95),
        ...
        for e in range(E):
            for batch_idx, (data, target) in enumerate(self.data_iterator):
                ...
                for name, p in self.model.named_parameters():
                    if p.ndim >= 2:
                        p.grad.data = (1 - args.gamma) * p.grad + args.gamma * ps_c[name]/((args.K)*lr*args.alpha)
                torch.nn.utils.clip_grad_norm_(parameters=self.model.parameters(), max_norm=10)
                self.optimizer.step()
                #for n, p in self.model.named_parameters():
                #    if not p.requires_grad:
                #        continue
                #    p.data.add_(ps_c[n].mul(args.gamma/(args.K)))
    ...
    def update_FedMuonA_SVD(self, weights, E, index,momen_m, lr,step):
        self.model.load_state_dict(weights)
        self.model.to(device)
        self.data_id_loader(index)
        ...
        param_groups = [
            dict(params=hidden_weights, use_muon=True,
                 lr=lr*args.alpha, weight_decay=0.01,momentum=0.98),
            dict(params=hidden_gains_biases + nonhidden_params, use_muon=False,
                 lr=lr, betas=(0.9, 0.95), weight_decay=0.01),
        ]
        self.optimizer = SingleDeviceMuonWithAuxAdam(param_groups)

        momen_svd = {}  # {param_name: {'U','S','Vh','orig_shape'}}
        momen_recon = {}  # {param_name: reconstructed momentum on CPU (fp32)}
        ...
        for group in self.optimizer.param_groups:
            for p in group['params']:
                ...
                if p.ndim >= 2:
                    mb = state['momentum_buffer'].detach().to('cpu', dtype=torch.float32)
                    mb_2d, orig_shape = _flatten_for_svd(mb)
                    U, S, Vh = torch.linalg.svd(mb_2d, full_matrices=False)
                    m, n = mb_2d.shape
                    target_compression_ratio=0.1
                    r = compute_rank_for_compression(m, n, target_compression_ratio)
                    U, S, Vh = U[:, :r], S[:r], Vh[:r, :]
                    momen_svd[param_name] = {'U': U, 'S': S, 'Vh': Vh, 'orig_shape': orig_shape}
        ...
        if index % 10 == 0:
            print('norm:', norm,'loss:',self.loss)
        self.R=self.R+1
        return delta_w,momen_svd

    def update_FedMuonA(self, weights, E, index,momen_m, lr,step):
        self.model.load_state_dict(weights)
        self.model.to(device)
        self.data_id_loader(index)
        ...
        for group in self.optimizer.param_groups:
            for p in group['params']:
                ...
                if p.ndim >= 2:
                    ...
                    U, S, Vh = torch.linalg.svd(mb_2d, full_matrices=False)
                    m, n = mb_2d.shape
                    target_compression_ratio=args.beta1
                    r = compute_rank_for_compression(m, n, target_compression_ratio)
                    U, S, Vh = U[:, :r], S[:r], Vh[:r, :]
                    momen_m[param_name] = {'U': U, 'S': S, 'Vh': Vh, 'orig_shape': orig_shape}
        if args.lora == 1:
            delta_w = {k: v.cpu() for k, v in self.model.state_dict().items() if 'lora' in k}
            for k, v in self.model.state_dict().items():
                if 'lora' in k:
    ...
    def update_SAM(self, weights, E, index, lr):
        ...
        if args.optimizer == 'AdamW':
            ...
            self.optimizer = SAM(self.model.parameters(), base_optimizer, lr=lr, weight_decay=0.01, rho=args.rho)
        self.loss = 0
        for e in range(E):
            for batch_idx, (data, target) in enumerate(self.data_iterator):
                data = data.to(device)
    ...
    def update_NSAM(self, weights, E, index, ps_c, lr):
        ...
        base_optimizer = torch.optim.SGD
        self.optimizer = SAM(self.model.parameters(), base_optimizer, lr=lr, weight_decay=0.001, rho=args.rho)
        #self.optimizer = FastSAM(self.model.parameters(), base_optimizer, lr=lr, momentum=0.9, weight_decay=0.001, rho=args.rho)
        step = 0  # 新增步数计数
        self.loss = 0
        for e in range(E):
            for batch_idx, (data, target) in enumerate(self.data_iterator):
    ...
    def update_FedAdam_mini(self, weights, E, index, lr):
        self.model.load_state_dict(weights)
        self.model.to(device)
        self.data_id_loader(index)
        if args.freeze==0:
        ...
        self.optimizer = Adam_mini(
            ...
            eps=1e-8,
            weight_decay=0.01,
            dim=96,  # Swin-Tiny 初始 embedding dim 是 96
            n_heads=3,  # 第一层有 3 个 attention 头
            verbose=False,
        )
    ...
    def update_FedLion(self, weights, E, index, lr):
        self.model.load_state_dict(weights)
        self.model.to(device)
        self.data_id_loader(index)
        if args.freeze==0:
            for name, param in self.model.named_parameters():
                if "classifier" in name or "head" in name:
                    param.requires_grad = True
        self.optimizer =  Lion(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr, weight_decay=0.001)
        step = 0  # 新增步数计数
        self.loss=0
        for e in range(E):
            for batch_idx, (data, target) in enumerate(self.data_iterator):
    ...
    def update_FedMuon(self, weights, E, index, lr):
        ...
        self.R=self.R+1
        return delta_w
    def update_FedMARS(self, weights, E, index, lr):
        self.model.load_state_dict(weights)
        self.model.to(device)
        self.data_id_loader(index)
        if args.freeze==0:
            for name, param in self.model.named_parameters():
                if "classifier" in name or "head" in name:
                    param.requires_grad = True
        self.optimizer= MARS(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr, betas=(0.95, 0.99), gamma=0.025,weight_decay=0.01)
        step = 0  # 新增步数计数
        self.loss=0
        for e in range(E):
            for batch_idx, (data, target) in enumerate(self.data_iterator):
    ...
    def load_dict(self):
        self.func_dict = {
            ...
            'FedAvg_adamwM': self.update_fedavg_adamw,
            'FedNSAM': self.update_NSAM,
            'FedAdamW': self.update_FedAdamW,
            'FedLion': self.update_FedLion,
            'FedAdam_mini': self.update_FedAdam_mini,
            'FedMARS': self.update_FedMARS,
            'FedMuon': self.update_FedMuon,
            'FedMuonC': self.update_FedMuonC,
            'FedMuonA':self.update_FedMuonA,
            'FedMuonAC':self.update_FedMuonAC,
            'FedMuonA_SVD':self.update_FedMuonA_SVD,
    ...
    def update_func(self, alg, weights, E, index, lr, ps_c=None, v=None, step=None, shared_state=None,ci=None):
        self.load_dict()
        if alg in {'FedLESAM','FedCM','FedACG', 'FedNSAM', 'FedSARAH', 'FedAdamL', 'FedNadam', 'FedANAG','FedNCM',
                   'FedAvg_Nadamw','FedadamW_CM','FedGN','IGFL','FedNesterov','MoFedSAM','FedAvg_Nadamw','FedAdamL','FedAdamWc','FedMuonC','FedMuonAM'}:
        ...
        if alg in {'FedAvg_adamw_A', 'FedAvg_adamw_M','FedAvg_adam_A','FedLADA','FedAdamW','FedAdamSAM','FedMuonAC','FedMuonAC_SVD'}:
            return self.func_dict.get(alg, None)(weights, E, index, ps_c, v, lr, step)
        if alg in {'FedAvg_adamw_V','FedAvg_adamw_V2','FedAvg_adamw_AV','FedMuonA','FedMuonA_SVD'}:
...
@torch.no_grad()
def apply_weights_avg_muon(num_workers, weights,model):
    ...
    update = zeropower_via_newtonschulz5(sum_weights, steps=ns_steps)
    update *= max(1, grad.size(-2) / grad.size(-1)) ** 0.5
...
if __name__ == "__main__":
    ...
    for epochidx in range(epoch_s, epoch):
        ...
        if alg in { 'FedAdamL', 'FedCM','FedadamW_CM','FedGN','IGFL','MoFedSAM','FedAdamL','FedNCM'}:
            weights_and_ci = []
            ...
            del weights_and_ci
        if alg in {'FedAdamWc','FedMuonC','FedMuonAM'}:
            weights_and_ci = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
                weights_and_ci = weights_and_ci + [worker.update_func.remote(alg, current_weights, E, idx, lr, ps_c)
                                                   for
                                                   worker, idx in
                                                   zip(workers, index_sel)]
            weights_and_ci = ray.get(weights_and_ci)
        ...
        if alg in {'FedLADA','FedAvg_adamw_A','FedAdamW','FedAdamSAM','FedMuonAC'}:
            weights_and_ci = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
        ...
        if alg in {'SCAFFOLD','SCAFFOLD+'}:
        ...
        elif alg in {'FedAvg_adamw_M','FedAvg_adam_A','FedAvg_adamw_V','FedAvg_adamw_V2','FedAvg_adamw_AV','FedMuonA'}:
            weights_and_ci = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
            ...
            for i in range(0, n, int(n / args.p)):
                ...
                weights_and_ci = weights_and_ci + [
                    ...
                    zip(workers, index_sel)]
            weights_and_ci=ray.get(weights_and_ci)
            current_weights,v=apply_weights_avg_svd(num_workers, weights_and_ci,model)
            model.load_state_dict(current_weights)
            #del weights
            #step.add_(nums_sample / args.batch_size * args.E)
            step.add_(args.K)
        ...
        elif alg in {'FedAvg', 'FedSAM', 'Fedprox', 'FedAvg_adam', 'FedAvg_adamw', 'FedAdamP',
                     'FedAvg_momentum','FedAvg_adamw_L','MOON','Fedspeed','FedLion','FedAdam_mini','FedAvg_nadamw','FedMARS','FedMuon'}:
            weights = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
        ...
        elif alg in {'FedMoment','FedAvg_adamwM'}:
            weights = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
        ...
        if alg in { 'FedACG', 'FedNSAM','FedNesterov','FedAvg_Nadamw'}:
            weights = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
